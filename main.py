...
#             if bot_platform.ycor() + bot_platform.random_collision_location() >= ball.ycor():
#                 screen.update()
#                 bot_platform.move_bot_down()
#                 #screen.update()
#             elif bot_platform.ycor() + bot_platform.random_collision_location() <= ball.ycor():
#                 screen.update()
...
#             game_check = False
random_int = random.randint(0, 65)
#elif choice == "1":
while game_check:  # Цикл для игры против ИИ
    ...
    if ball.xcor() < -100 and ball.ycor() < 0:
        if bot_platform.ycor() - bot_platform.random_int > ball.ycor():
            screen.update()
            bot_platform.move_bot_down()
            screen.update()
        elif bot_platform.ycor() + bot_platform.random_int < ball.ycor():
            screen.update()
            bot_platform.move_bot_up()
            screen.update()
    elif ball.xcor() < -100 and ball.ycor() > 0:
